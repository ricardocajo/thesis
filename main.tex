% !TEX program = xelatex
% !BIB program = bibtex
\documentclass[11pt, openright, twoside]{report}
\usepackage[english,portuguese]{babel}

\selectlanguage{english}

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% %%%%%%%%%%%%%%%%%%%% IMPORTANT: SET THE INFORMATION %%%%%%%%%%%%%%%%%%%%%%%%
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\departmentname}{INFORMÁTICA}
\newcommand{\thesisname}{RoboFix: Formalização de invariantes para robots}
\newcommand{\studentname}{Ricardo Jorge Dias Cordeiro}
\newcommand{\coursename}{Engenharia Informática}
\newcommand{\specname}{Designação da Especialização / Perfil, se aplicável}
\newcommand{\versaoprovisoria}{Versão Provisória} % TODO: Blank for final version
\newcommand{\thesistype}{Dissertação}
\newcommand{\firstadvisor}{Alcides Miguel Cachulo Aguiar Fonseca}
\newcommand{\secondadvisor}{Se aplicável, nome completo do segundo orientador}
\newcommand{\thirdadvisor}{Se aplicável, nome completo do terceiro orientador}
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% -----------------------------------------------------------------------------
% Packages defined by the student
\input{packages}
% -----------------------------------------------------------------------------

% Index
\makeindex

% -----------------------------------------------------------------------------
% Begin of the thesis
\begin{document}

\startpagecounter{roman}

% Cover
\input{./chapters/cover}
\cleanpage

% Acknowledgements
\input{./chapters/acknowledgements}
\cleardoublepage 

% Resumo em Portugues
\input{./chapters/resumo_pt}
\cleardoublepage

% Abstract in English
\input{./chapters/resumo_ing}
\cleardoublepage

\pagestyle{plain}

% Table of Contents
\tableofcontents

% All the remaining lists
\listoffigures
\addcontentsline{toc}{chapter}{List of Figures}

\listoftables
\addcontentsline{toc}{chapter}{List of Tables}

\lstlistoflistings
\addcontentsline{toc}{chapter}{Listings}

\cleardoublepage

% ---------------------------------------------------------------------
% Thesis content
\startpagecounter{arabic}

\input{./chapters/introduction}
\input{./chapters/background}
\input{./chapters/approach}

% Appendixes
\appendix
\input{./chapters/appendixes}

% Bibliography
\bibliography{references}
\addcontentsline{toc}{chapter}{References}

\end{document}